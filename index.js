...
// ---------------------------------------------------------------------
// 0. NAVIGATION PAGE / working page link
// ---------------------------------------------------------------------
  ...
  try {
    ...
    result.rows.forEach((row) => {
      ...
      if (row.category_id === 1)
        grouped.entrees.push({ name: row.name, price });
      else if (row.category_id === 3)
        grouped.a_la_carte.push({ name: row.name, price });
      else if (row.category_id === 4)
        grouped.sides.push({ name: row.name, price });
      else if (row.category_id === 2)
        grouped.appetizers.push({ name: row.name, price });
    });
  ...
  try {
    ...
    result.rows.forEach((row) => {
      ...
      if (row.category_id === 1) menu.entrees.push({ name: row.name, price });
        ...
        menu.sides.push({ name: row.name, price });
    });
  } catch (err) {

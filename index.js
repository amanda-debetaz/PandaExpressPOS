/**
 * @file Main application entry point for Panda Express POS System
 * @description Express.js server with Passport authentication, Prisma ORM, and multiple view routes
 * @module index
 ...
 * @async
 * @function shutdown
 * @param {string} signal - The signal received (SIGINT, SIGTERM, etc.)
 * @description Disconnects Prisma client and exits process cleanly
 ...
 * @description Auto-logout check runs every 60 seconds for 30-minute inactivity
 */
setInterval(() => {
 ...
 * @param {string|string[]|undefined} allowedRole - Role(s) required to access the route. Managers have access to all routes.
 * @returns {Function} Express middleware function
 * @description Checks if user is authenticated and has required role. Managers bypass role checks.
 * @example
 ...
 * @query {string} next - URL to redirect after successful login
 * @description Displays login page with Google OAuth and local login options
 */
...
/**
 * POST /login - Handle local authentication
 * @route POST /login
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @param {express.NextFunction} next - Express next middleware function
 * @body {number} employee_id - Employee ID for authentication
 * @body {string} password_hash - Password for authentication
 * @body {string} next - URL to redirect after successful login
 ...
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @query {string} state - Encoded redirect URL
 * @description Processes Google OAuth response and redirects authenticated user
 */
app.get(
...
/**
 * GET / - Home/navigation page
 * @route GET /
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @description Renders main navigation page with role-based access buttons
 */
app.get("/", (req, res) => {
 ...
 * @param {express.Response} res - Express response object
 * @access Manager only
 * @description Displays manager dashboard with employee management and reporting features
 */
...
/**
 * GET /cashier - Cashier POS interface
 * @route GET /cashier
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 ...
 * @type {Date}
 * @description Orders marked 'done' before this date are hidden from kitchen display
 */
let doneViewCutoff = new Date("2025-11-08T00:00:00Z");

/**
 * GET /kitchen - Kitchen display system
 * @route GET /kitchen
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @access Cook role required
 * @description Renders kitchen view with queued, cooking, and completed orders.
 *              Includes prepared item stock tracking and batch cooking interface.
...
/**
 * POST /kitchen/:orderId/status - Update order status in kitchen
 * @route POST /kitchen/:orderId/status
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @param {number} req.params.orderId - Order ID to update
 * @body {string} status - New status ('queued', 'prepping', 'done')
 * @access Cook role required
 * @description Updates order status and manages store statistics. When marking 'done',
 *              increments daily statistics. When unmarking from 'done', decrements statistics.
 *              Also handles prepared item stock consumption.
 * @returns {void} Redirects to /kitchen or sends error
...
/**
 * POST /kitchen/:orderId/cancel - Cancel/delete an order
 * @route POST /kitchen/:orderId/cancel
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @param {number} req.params.orderId - Order ID to cancel
 * @access Cook role required
 * @description Permanently deletes an order and all related items/payments via cascade
 * @returns {void} Redirects to /kitchen or sends error
 * @throws {500} If database operation fails
 */
...
/**
 * POST /kitchen/clear-done - Clear completed orders from view
 * @route POST /kitchen/clear-done
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @access Cook role required
 * @description Sets cutoff date to now, hiding older completed orders from kitchen display
 * @returns {void} Redirects to /kitchen
 */
...
/**
 * Set of active SSE client response streams for kitchen updates
 * @type {Set<express.Response>}
 */
const kitchenClients = new Set();

/**
 * Broadcast event to all connected kitchen SSE clients
 * @function kitchenBroadcast
 * @param {string} event - Event name (e.g., 'queued-changed')
 * @param {Object} data - Event payload data
 * @description Sends Server-Sent Events to all kitchen display clients for real-time updates
 */
...
/**
 * GET /kitchen/events - Server-Sent Events stream for kitchen
 * @route GET /kitchen/events
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @description Establishes SSE connection for real-time kitchen order updates.
 *              Sends keep-alive pings every 15 seconds.
 * @returns {void} Keeps connection open for streaming events
 */
...
/**
 * GET /kitchen/queued-count - Get count of queued orders
 * @route GET /kitchen/queued-count
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @description Returns count of orders with 'queued' status for auto-refresh polling
 * @returns {Object} JSON with count property
 * @returns {number} returns.count - Number of queued orders
...
/**
 * GET /menu-board - Customer-facing digital menu board
 * @route GET /menu-board
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @query {string} page - Category to display ('entrees', 'a_la_carte', 'sides', 'appetizers', 'featured')
 * @description Displays menu items grouped by category with automatic page cycling for digital signage
 * @returns {void} Renders menu-board view
 * @throws {500} If database query fails
 */
...
// ---------- 1. KIOSK MENU ----------
/**
 * Menu cache object for kiosk display
 * @type {Object}
 * @property {Array} entrees - Entree menu items
 * @property {Array} a_la_carte - A la carte menu items
 * @property {Array} sides - Side menu items
 * @property {Array} appetizers - Appetizer menu items
 * @property {Array} drinks - Beverage menu items
 */
let menuCache = { entrees: [], a_la_carte: [], sides: [], appetizers: [] , drinks: []};

/**
 * GET /kiosk - Self-service kiosk ordering interface
 * @route GET /kiosk
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @access Authenticated users
 * @description Displays customer-facing kiosk menu with items, allergens, nutrition info,
 *              size pricing for premium items, and combo builders. Includes dynamic pricing
 *              from database for different sizes (small/medium/large).
 * @returns {void} Renders kiosk view with categorized menu
 * @throws {500} If database query fails
 */
 ...
 * @access Authenticated users
 * @description Adds item to server-side session cart. If item already exists, increments quantity.
 * @returns {Object} JSON with success status and updated cart
 * @throws {400} If name or price is missing
 */
 ...
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @description Returns current cart items from session
 * @returns {Object} JSON with cart array
 */
...
/**
 * DELETE /api/cart/clear - Clear session cart
 * @route DELETE /api/cart/clear
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @description Empties cart from session
...
/**
 * POST /api/checkout - Process kiosk order checkout
 * @route POST /api/checkout
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @body {Array} cart - Array of cart items
 * @body {string} paymentMethod - Payment method (credit, debit, cash, mobile)
 * @body {string} dineOption - Dining option ('dine_in' or 'takeout')
 * @access Authenticated users
 * @description Creates order, order_items, order_item_options, and payment records.
 *              Calculates tax from database, handles combo items and a la carte items,
 *              stores per-item tax amounts.
 * @returns {Object} JSON with order details (orderId, total, tax)
 * @throws {400} If cart is empty, payment method missing, or invalid dine option
 * @throws {500} If database operation fails
 */
...
/**
 * POST /api/clock/in - Clock in employee for cashier shift
 * @route POST /api/clock/in
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @body {number} employee_id - Employee ID to clock in
 * @access Cashier role required
 * @description Verifies employee exists and is active, then returns employee info for session tracking
 * @returns {Object} JSON with success status and employee details
 * @throws {400} If employee_id missing or employee inactive
...
/**
 * POST /api/clock/out - Clock out employee from cashier shift
 * @route POST /api/clock/out
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @access Cashier role required
 * @description Simple endpoint to confirm clock out (employee tracking handled client-side)
 * @returns {Object} JSON with success status
...
/**
 * POST /api/cashier/checkout - Process cashier POS checkout
 * @route POST /api/cashier/checkout
 * @async
 * @param {express.Request} req - Express request object
 * @param {express.Response} res - Express response object
 * @body {Array} cart - Array of order items
 * @body {string} paymentMethod - Payment method (cash, card, giftcard, dining_dollars, meal_swipe)
 * @body {string} dineOption - Dining option ('dine_in' or 'takeout')
 * @body {number} clockedInEmployeeId - Cashier employee ID
 * @access Cashier role required
 * @description Creates order with cashier-specific workflow. Orders are marked as 'done' immediately.
 *              Handles combos, a la carte items, calculates tax from database, associates order with employee,
 *              and updates daily store statistics.
 * @returns {Object} JSON with success status and order ID
 * @throws {400} If cart empty or invalid payment method
 * @throws {500} If database operation fails
 */
